...
<cfelseif isDefined("error.Diagnostics") AND findNoCase("There are no stored procedures", error.Diagnostics)>
	<p><cfoutput>#error.diagnostics#</cfoutput>
<cfelseif isDefined("error.Diagnostics") AND findNoCase("SELECT command denied", error.Diagnostics) AND findNoCase("for table 'proc'", error.Diagnostics)>
	<p>GangPlank requires that the database account used has <em>SELECT</em> Permission on the table <em>mysql.proc</em>. See documentation for more information.</p>
	<p><cfoutput>#error.rootcause.detail#</cfoutput>
<cfelse>
